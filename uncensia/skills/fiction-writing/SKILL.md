...
Write the requested work directly using the conversation, supplied text, and any saved story notes. A character card, outline, or settings form is optional. Ask only for missing information that materially changes the work; otherwise use the user's premise and develop it.

Separate three kinds of context, following the [established writing workflows](references/sources.md): stable character/world facts, events already established in the manuscript, and the user's current direction for the next passage. Examples demonstrate voice; they are not past events. Read an attached source when the work depends on it, and distinguish its contents from new invention. For fanfiction, follow the supplied continuity and the user's departures from canon.
