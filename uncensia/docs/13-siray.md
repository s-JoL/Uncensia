...
内置适配器的 Base URL 为 `https://api.siray.ai/v1`，Bearer key 经 SecretVault 加密保存。
内置集成只登记以下 Spicy 型号；官方目录的 Seedance 名为 **2.5 Spicy**，没有在名称中补造 Pro。

首次安装的模型选择见 [运行维护](12-operations.md)。
Siray 默认启用 Seedream 文生图、Seedream 编辑与 Wan 的三个入口，并作为托管默认绑定：文生图绑 Seedream 5.0 Pro Spicy T2I、改图绑 Seedream 5.0 Pro Spicy I2I、视频绑 Wan 3.0 T2V。Qwen 编辑与 Seedance 全系默认不启用，可从模型目录添加。
Wan 首尾帧、多参考另外开放具名 agent 工具；默认文生图、默认编辑与默认视频直接使用各自的通用工具（`generate_image`／`edit_image`／`generate_video`），不重复挂名。
已有安装仍保留自己的启用状态、参数、默认绑定及自定义提示词，不在启动时覆盖。密钥由各安装单独配置。

| 型号（含供应商前缀） | 操作 | 来源与输出参数 |
|---|---|---|
| `bytedance/seedream-5.0-pro-t2i-spicy` | 文生图 | 无原图；16 种尺寸；jpg/png |
...
编辑图片没有接入 `/images/edits`：所有图片操作（文生图与两款编辑型号）都提交到 `/images/generations/async`，视频提交到 `/video/generations`。
...
`scripts/audit-siray.ts` 无费用验证模型发现只暴露显式支持的 `siray-media` 档案，以及各操作的请求路径、字段、来源顺序、两种响应结构、失败、不确定交付、结果存储、凭证隔离和恢复不重复提交。
...
- [Seedream 文生图 OpenAPI](https://docs.siray.ai/api-reference/openapi-spec/seedream-5.0-pro-t2i-spicy.json)
